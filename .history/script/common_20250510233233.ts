import {
  ...
  // Check if we're dealing with a browser wallet from the React hook
  if (typeof wallet.getUtxos !== 'function') {
    // Browser wallet from React hook
    ...
    try {
      ...
      // Some wallets might require different way to get collateral
      const collateralUtxos = await wallet.getCollateral();
      collateral = collateralUtxos[0];
    }
