...
export const main = async (
  ...
  try {
    console.log("Starting IPFS operations...");
    
    // Example data to upload
    const testData = {
      title: "Test Data",
      description: "This is a test data object for IPFS upload",
      timestamp: new Date().toISOString(),
      items: [1, 2, 3, 4, 5]
    };
    
    console.log("Uploading regular data to IPFS...");
    // Upload data
    const uploadUrl = await uploadToIPFS(testData, pinataJWT, pinataGateway);
    console.log(`Data uploaded successfully to: ${uploadUrl}`);
    
    // Fetch the uploaded data
    console.log("Fetching the uploaded data...");
    const fetchedData = await fetchFromIPFS(uploadUrl);
    console.log("Fetched data:", fetchedData);
    
    // Example with encrypted data (simulated here)
    console.log("Simulating encrypted data upload...");
    const simulatedEncryptedData = `ENCRYPTED:${btoa(JSON.stringify(testData))}`;
    
    // Upload encrypted data
    const encryptedUrl = await uploadEncryptedData(simulatedEncryptedData, pinataJWT, pinataGateway);
    console.log(`Encrypted data uploaded successfully to: ${encryptedUrl}`);
    
    // Fetch the encrypted data
    console.log("Fetching the encrypted data...");
    const fetchedEncryptedData = await fetchFromIPFS(encryptedUrl, false);
    console.log("Fetched encrypted data:", fetchedEncryptedData);
...
main();
// Uncomment and provide your actual JWT and gateway to run the main function
// main("YOUR_ACTUAL_JWT_TOKEN", "gateway.pinata.cloud");
...
if (process.argv.length >= 4) {
  ...
  main(jwt, gateway)
    .then(() => console.log("Main execution completed"))
    .catch(err => console.error("Main execution failed:", err));
}